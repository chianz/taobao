...
<div id="content">
    <div id="content-header">
        <div id="breadcrumb">
            ...
            <a href="/index.php/Admin/Article/index" class="current">文章管理</a>
            <a href="#" class="current"><?php if(empty($one["id"])): ?>添加<?php else: ?>修改<?php echo ($one["title"]); endif; ?>文章</a>
        </div>
    </div>
    <div class="row-fluid">
        <div class="span12">
            <div class="widget-box">
                <div class="widget-title"><span class="icon"> <i class="icon-info-sign"></i> </span>
                    <h5>
                        <?php if(empty($one["id"])): ?>添加<?php else: ?>修改<?php echo ($one["title"]); endif; ?>文章
                    </h5>
                </div>
                <div class="widget-content">
                    <form class="form-horizontal" method="post" action="/index.php/Admin/Article/update" name="basic_validate" id="basic_validate"
                          novalidate="novalidate">
                        <input type="hidden" name="id" value="<?php echo ($one["id"]); ?>">
                        <div class="control-group">
                            <label class="control-label">标题</label>
                            <div class="controls">
                                <input type="text" name="title" value="<?php echo ($one["title"]); ?>" class="span6">
                            </div>
                        </div>
                        <div class="control-group">
                            <label class="control-label">排序</label>
                            <div class="controls">
                                <input type="text" name="sort" value="<?php echo ((isset($one["sort"]) && ($one["sort"] !== ""))?($one["sort"]):999); ?>">
                            </div>
                        </div>
                        <div class="control-group">
                            ...
                            <div class="controls">
                                <select name="col" class="span9">
                                    <?php $temp=D("articlecolumn")->where(["sid"=>0])->select();?>
                                    <?php if(is_array($temp)): $i = 0; $__LIST__ = $temp;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; $temp1=D("articlecolumn")->where(["sid"=>$vo["id"]])->select();?>
                                    	<?php if(is_array($temp1)): $i = 0; $__LIST__ = $temp1;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo1): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo1["id"]); ?>" <?php if(($one["col"]) == $vo1['id']): ?>selected=selected<?php endif; ?>  ><?php echo ($vo["title"]); ?>-----<?php echo ($vo1["title"]); ?></option><?php endforeach; endif; else: echo "" ;endif; endforeach; endif; else: echo "" ;endif; ?>
                                </select>
                            </div>
                        </div>
                        <div class="control-group">
