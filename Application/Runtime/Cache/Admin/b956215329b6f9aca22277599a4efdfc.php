...
<div id="content">
    ...
    <div class="widget-content">
        <div><a href="/index.php/Admin/Menu/one" class="btn btn-success">增加菜单</a></div>
        <table class="table">
            <thead>
            <tr>
                <th>菜单名称</th>
            ...
            <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; if(($vo["sid"]) == "0"): ?><tr>
                        <td><?php echo ($vo["title"]); ?></td>
                        <td><?php echo ($vo["controller"]); ?></td>
                        <td><?php echo ($vo["function"]); ?></td>
                        <td><?php echo ($vo["parm"]); ?></td>
                        <td>
                            <?php if(($vo["is_show"]) == "1"): ?>是<?php else: ?>否<?php endif; ?>
                        </td>
                        <td><a href="/index.php/Admin/Menu/one?id=<?php echo ($vo["id"]); ?>" class="btn btn-primary btn-mini">修改</a><a href="javascript:if(confirm('确实要删除该内容吗?'))location='/index.php/Admin/Menu/del?id=<?php echo ($vo["id"]); ?>'" class="btn btn-danger btn-mini">删除</a></td>
                    </tr>
                    <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo1): $mod = ($i % 2 );++$i; if(($vo1["sid"]) == $vo['id']): ?><tr>
                                <td>┗ <?php echo ($vo1["title"]); ?></td>
                                <td><?php echo ($vo1["controller"]); ?></td>
                                <td><?php echo ($vo1["function"]); ?></td>
                                <td><?php echo ($vo1["parm"]); ?></td>
                                <td>
                                    <?php if(($vo1["is_show"]) == "1"): ?>是<?php else: ?>否<?php endif; ?>
                                </td>
                                <td><a href="/index.php/Admin/Menu/one?id=<?php echo ($vo1["id"]); ?>" class="btn btn-primary btn-mini">修改</a><a href="javascript:if(confirm('确实要删除该内容吗?'))location='/index.php/Admin/Menu/del?id=<?php echo ($vo1["id"]); ?>'" class="btn btn-danger btn-mini">删除</a></td>
